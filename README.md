...
Whenever an AI organize job starts, Organizer opens `activity/activity.html` in a tab (turn this off with "Show the AI activity page automatically" in Settings; it stays reachable from the popup's "View AI detail" button). The page live-polls the job and shows: a summary (method, provider, item count, state, timestamps); the sections sent to the AI with a per-section sent/sending/waiting indicator; and, once results land, every item next to the category it was given — grouped by category or as a flat list. Each category lists the sites its items have in common. The "why" column depends on the method: the built-in method reports the catalog terms or domain that matched; a vendor AI (your own key) is asked for a short per-item reason; Dave AI returns categories only, so each group's rationale is inferred from shared sites and words and labelled "inferred". The page also has Retry (re-runs the organize for that kind), Cancel (while active), and Undo — which for bookmarks does an exact restore of the backup taken just before organizing, and for tabs ungroups and returns every tab to its previous position.
