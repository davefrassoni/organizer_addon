...
document.querySelector("form").onsubmit = async event => { event.preventDefault(); const provider = $("#provider").value, method = $("#method").value, tabFallback = $("#tab-fallback").value, bookmarkScope = $("#bookmark-scope").checked ? "loose" : "all", excludeFoldersFromOrganizing = $("#exclude-folders").checked, organizeInsideExcludedFolders = $("#exclude-folders").checked && $("#organize-inside-excluded").checked, closeDuplicateTabs = $("#close-duplicate-tabs").checked, removeDuplicateBookmarks = $("#remove-duplicate-bookmarks").checked; try { const permissionRequest = method === "ai" && api.permissions ? OrganizerPermissions.request(provider) : Promise.resolve(true); const old = (await api.storage.local.get({ organizerSettings: DEFAULTS })).organizerSettings; if (!await permissionRequest) { $("#status").textContent = t("aiAccessNotGranted"); return; } const apiKeys = { ...(old.apiKeys || {}) }; if (provider !== "dave") apiKeys[provider] = $("#api-key").value.trim(); await api.storage.local.set({ organizerSettings: { method, provider, model: $("#model").value.trim(), apiKeys, tabFallback, bookmarkScope, excludeFoldersFromOrganizing, organizeInsideExcludedFolders, closeDuplicateTabs, removeDuplicateBookmarks } }); $("#status").textContent = t("savedStatus"); } catch (error) { $("#status").textContent = error.message || t("errorGeneric"); } };
OrganizerI18n.apply();
