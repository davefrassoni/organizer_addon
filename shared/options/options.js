...
async function load() { const data = await api.storage.local.get({ organizerSettings: DEFAULTS }); const value = { ...DEFAULTS, ...data.organizerSettings }; $("#ui-language").value = OrganizerI18n.SUPPORTED.includes(value.uiLanguage) || value.uiLanguage === "auto" ? value.uiLanguage : "auto"; $("#method").value = value.method; $("#tab-fallback").value = value.tabFallback; $("#bookmark-scope").checked = value.bookmarkScope !== "all"; $("#exclude-folders").checked = !!value.excludeFoldersFromOrganizing; $("#organize-inside-excluded").checked = !!value.organizeInsideExcludedFolders; $("#close-duplicate-tabs").checked = !!value.closeDuplicateTabs; $("#remove-duplicate-bookmarks").checked = !!value.removeDuplicateBookmarks; $("#open-activity").checked = value.openActivityOnStart !== false; $("#provider").value = value.provider; $("#model").value = value.model || ""; $("#api-key").value = value.apiKeys?.[value.provider] || ""; toggle(); toggleFolderScope(); }
...
document.querySelector("form").onsubmit = async event => { event.preventDefault(); const provider = $("#provider").value, method = $("#method").value, tabFallback = $("#tab-fallback").value, bookmarkScope = $("#bookmark-scope").checked ? "loose" : "all", excludeFoldersFromOrganizing = $("#exclude-folders").checked, organizeInsideExcludedFolders = $("#exclude-folders").checked && $("#organize-inside-excluded").checked, closeDuplicateTabs = $("#close-duplicate-tabs").checked, removeDuplicateBookmarks = $("#remove-duplicate-bookmarks").checked, openActivityOnStart = $("#open-activity").checked, uiLanguage = $("#ui-language").value; try { const permissionRequest = method === "ai" && api.permissions ? OrganizerPermissions.request(provider) : Promise.resolve(true); const old = (await api.storage.local.get({ organizerSettings: DEFAULTS })).organizerSettings; if (!await permissionRequest) { $("#status").textContent = t("aiAccessNotGranted"); return; } const apiKeys = { ...(old.apiKeys || {}) }; if (provider !== "dave") apiKeys[provider] = $("#api-key").value.trim(); await api.storage.local.set({ organizerSettings: { method, provider, model: $("#model").value.trim(), apiKeys, tabFallback, bookmarkScope, excludeFoldersFromOrganizing, organizeInsideExcludedFolders, closeDuplicateTabs, removeDuplicateBookmarks, openActivityOnStart, uiLanguage } }); $("#status").textContent = t("savedStatus"); } catch (error) { $("#status").textContent = error.message || t("errorGeneric"); } };
